...
function getAuthHeaders() {
    ...
    const apiKey = localStorage.getItem("apiKey") || "admin-secret-key";
    const headers = {
...
/**
 * Logout current user and clear token storage and offline user-specific data
 */
async function logoutUser() {
    ...
    if (window.OfflineDB && typeof window.OfflineDB.clearUserData === 'function') {
        try {
            await window.OfflineDB.clearUserData();
        } catch (e) {
            console.warn("Failed to clear offline user data on logout:", e);
        }
    }
    window.location.href = "index.html";
}
...
/**
 * Show a global stale-data warning banner when a page is rendering cached offline data.
 * @param {string} anchorElementId - ID of element to insert the banner before (optional)
 * @param {string|null} lastSyncedAt - ISO timestamp of last sync (or null)
 */
