"""Unified Facility Operational State Service."""
...
class OperationalStateService:
    """Aggregates operational state for healthcare facilities."""
    ...
    def publish_camera_telemetry(
        ...
        raw_data = {
            ...
            "location_type": (
                telemetry_in.location_type
                or "waiting_area"
            ),
            "source": "live_vision_pipeline",
        }

        sanitized = PrivacyLayer.sanitize_crowd_data(
            raw_data
        )
    ...
    @staticmethod
    def clear_camera_telemetry(
        facility_id: Optional[str] = None,
    ) -> None:

        if facility_id:
            _CAMERA_TELEMETRY_REGISTRY.pop(
                facility_id,
                None,
            )
        else:
    ...
    def publish_emergency_event(
        ...
        event: EmergencyEvent = validate_emergency_event(
            emergency_data
        )

        priority_score = (
            calculate_emergency_priority_score(event)
        )
    ...
    @staticmethod
    def get_emergency_load(
        facility_id: str,
    ) -> Optional[float]:

        record = _EMERGENCY_TELEMETRY_REGISTRY.get(
            facility_id
        )

        if (
            record
            and record["event"].emergency_active
            and record["event"].eligible
        ):
            return float(record["priority_score"])

        return None

    @staticmethod
    def clear_emergency_telemetry(
        facility_id: Optional[str] = None,
    ) -> None:

        if facility_id:
            _EMERGENCY_TELEMETRY_REGISTRY.pop(
                facility_id,
                None,
            )
        else:
    ...
    def get_facility_operational_state(
        ...
        if has_any_appointments:
            ...
            queue_length = waiting_appointments
            current_serving = 0
            people_present = waiting_appointments

            data_sources["queue"] = (
                "appointment_queue_database"
            )

        else:

            # Diagnostic booking fallback

            has_any_bookings = (
                db.query(DiagnosticBooking)
                .filter(
                    DiagnosticBooking.facility_id
                    == facility_id
                )
                .first()
                is not None
        ...
        telemetry = (
            OperationalStateService.get_camera_telemetry(
                facility_id
            )
        )
        ...
        if telemetry is not None:
            ...
            current_crowd: Optional[int] = (
                telemetry.get("people_count")
            )
        ...
        if queue_length is not None:
            ...
            try:
                ...
                data_sources["prediction"] = (
                    "person3_ml_wait_model_v3"
                )
        ...
        emergency_load = (
            OperationalStateService.get_emergency_load(
                facility_id
            )
        )
        ...
        if emergency_load is not None:
            ...
            data_sources["emergency"] = (
                "emergency_priority_engine"
            )
