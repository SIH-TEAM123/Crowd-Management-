...
const VIZITOR_VIZITOR_HEALTHCARE_API_BASE =
    window.API_BASE_URL ||
    window.VIZITOR_HEALTHCARE_API_BASE ||
    (
...
async function healthcareFetch(endpoint, options = {}) {
    ...
    const response = await fetch(
        `${VIZITOR_VIZITOR_HEALTHCARE_API_BASE}${endpoint}`,
        {
            ...options,
            headers: {
                ...healthcareHeaders(),
...
async function loadHealthcareProfile() {
    ...
    try {
        ...
        const welcomeHeading = document.getElementById("healthcareWelcome");
        if (welcomeHeading && !welcomeHeading.textContent.trim()) {
            welcomeHeading.textContent = "Patient Report";
        }
